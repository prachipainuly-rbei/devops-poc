...
cdbElinkRowSelection.prototype.rowHandler = function(event){
	...
	if(target.attr('id')!==undefined){
		app.select(target.attr('id'),event.ctrlKey,event.shiftKey);
	} else {
		app.select(target.data('cdb-elink-row-vindex'),event.ctrlKey,event.shiftKey,target.data('cdb-elink-cdb_object_id'));
	}
};
...
cdbElinkRowSelection.prototype.push = function (row,row_object_id,vindex_mode) {
	var elem = $('#' + row,this.context);
  if(row_object_id !== undefined){
      elem = $('tr[data-cdb-elink-cdb_object_id='+row_object_id+']',this.context);
  }
  if(vindex_mode !== undefined){
      elem = $('tr[data-cdb-elink-row-vindex='+row+']',this.context);
  }
  if(elem && elem.length){
...
cdbElinkRowSelection.prototype.resetSelection = function () {
    ...
    if(count<this.length){
		$.each(this.selected, function (index, value) {
			...
			elem = $("tbody tr[data-cdb-elink-row-vindex="+value+"]",this.context).removeClass('row_selected');
			count+=1;
		});
    }
	this.selectedIDs = {};
...
cdbElinkRowSelection.prototype.getSelectedRows = function(){
	...
	if(count<app.length){
		for(row in app.selected){
			elem = $('#'+row,app.context);
			if(elem === undefined){
				elem = $('tr[data-cdb-elink-row-vindex='+row+']',app.context);
			}
			ids.push(row);
			selected_row_dict[row] = elem;
		}
	}
	return {"ids":ids,"elems":selected_row_dict,'count':app.length};
};
