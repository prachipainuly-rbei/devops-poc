<metal:title define-macro="widget_title"
             tal:define="project obj|nothing;
             			 plugin filter(lambda x: True if x[1].__folder_content_class__== 'cdbpcs_project' else False, plugins)[0][-1]">
		<tal:dummy tal:define="kosmodromtools plugin.get_kosmodrom_tools()|nothing">
...
<metal:widget_dropdown define-macro="widget_dropdown"
  ...
  <metal:menu use-macro="global_plugins['components'].macros.operations_dropdown_menu"
    ...
    <metal:others fill-slot="pre_list"
    			  tal:define="kosmodromtools kosmodromtools|plugin.get_kosmodrom_tools()">
	        ...
	        <div class="projectdetail-info">
	          <div class="container-fluid">
	            ...
	            <div class="row-fluid detailinfo-label">
	              ...
	              <div class="span6 fix-text muted">${project.Label('mapped_category_name')}</div>
	            </div>
	            <div class="row-fluid">
	              <div class="span6 fix-text">${project.mapped_customer_name}</div>
	              <div class="span6 fix-text">${project.mapped_category_name}</div>
	            </div>
	            <div class="row-fluid detailinfo-label">
