...
import CalendarSimpleConverter from '../CalendarSimpleConverter.js';
import moment from 'moment';

describe('The CalendarSimpleConverter class', () => {

    const backendFormat = 'DD.MM.YYYY';
    const frontendFormat = 'YYYY-MM-DD HH:mm:ss';
    const frontendDateOnlyFormat = 'YYYY-MM-DD';
    const converter = new CalendarSimpleConverter(
    ...
    it('parses value format ">=fromDate AND <=toDate"', () => {
        expect.assertions(9);
        const dateStrS1 = '>=08.03.2018 AND <=15.03.2018';
        const dateStrS2 = '>=08.03.2018 AND <=08.03.2018';
        const dateStrS3 = '>08.03.2018 AND <15.03.2018';
        ...
        expect(converter.convertToDisplay(dateStrS1)).toBe('2018-03-08 - 2018-03-15');
        expect(converter.convertToDisplay(dateStrS2)).toBe('2018-03-08');
        expect(converter.convertToDisplay(dateStrS3)).toBe('2018-03-09 - 2018-03-14');
        expect(converter.convertToDisplay(dateStrF1)).toBe('');
        expect(converter.convertToDisplay(dateStrF2)).toBe('');
        expect(converter.convertToDisplay(dateStrF3)).toBe('');
        expect(converter.convertToDisplay(dateStrF4)).toBe('');
        expect(converter.convertToDisplay(dateStrF5)).toBe('');
        expect(converter.convertToDisplay(dateStrF6)).toBe('');
    });
    ...
    it('checks whether the input is valid', () => {
        ...
        const dateStrS3 = '2018-03-08 - 2018-03-15';
        const dateStrS4 = '2018-03-15 - 2018-03-08';
    ...
    it('converts input in format "fromDate - toDate" to value', () => {
        expect.assertions(1);
        const dateStr = '2018-03-08 - 2018-03-15';
