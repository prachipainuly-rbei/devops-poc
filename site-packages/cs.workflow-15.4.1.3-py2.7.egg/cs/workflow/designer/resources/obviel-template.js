// CON: Patched module.Template.prototype.finalize to defer image loading
/*
How does this work?
...
(function($, module) {
    ...
    var IMG_PLACEHOLDER = 'data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=';
    ...
    module.Template.prototype.finalize = function(el) {
        ...
        function replaceDataAttr(el, name) {
            var value = getDirective(el, 'data-' + name);
            if (value !== null) {
                el.setAttribute(name, value);
            }
        }

        function replaceDataSrcAttr(el) {
            var name = 'src';
            if (!el.getAttribute(name)) {
                // render using empty image immediately
                el.setAttribute(name, IMG_PLACEHOLDER);
            }
            return function() {
                // load real image after initial rendering
                replaceDataAttr(el, name);
            };
        }
        ...
        $('*.obviel-template-special-attr', el).each(function() {
            ...
            for (name in SPECIAL_ATTR) {
                if (name === 'src') {
                    setTimeout(replaceDataSrcAttr(this), 0);
                } else {
                    replaceDataAttr(this, name);
                }
            }
